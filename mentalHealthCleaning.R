...
#load data
data <- read.csv("./Data/Behavioral_Risk_Factor_Data__Health-Related_Quality_of_Life__HRQOL_.csv")
...
td <- data%>%filter(Question == 'Mean mentally unhealthy days',Break_Out == 'Overall',!is.na(Year))%>%
  group_by(LocationDesc,Year,LocationAbbr)%>%summarize(m = mean(Data_Value))
